...
const SEOHead = ({
  ...
  useEffect(() => {
    ...
    if (ogUrl) ogUrl.setAttribute('content', `${url}${location.pathname}`);
    ...
    if (twitterUrl) twitterUrl.setAttribute('content', `${url}${location.pathname}`);
    
    // Update canonical URL
    const canonicalLink = document.querySelector('link[rel="canonical"]');
    if (canonicalLink) {
      canonicalLink.setAttribute('href', `${url}${location.pathname}`);
    }
